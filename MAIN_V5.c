...
int main(void)
{ 
	short angle, distance, angle1, angle2, dist1, dist2, qualite,angle090;
	int i=0, x = 0, y = 0, j = 0;
	PWM_Lidar();
	Init_UART();
	Init_UART1();
				...
				if (((tabR[i]&0x3E)==0x3E)&&((tabR[i+1]&0x01)==0x01)){					//masquage des trames de mauvaise qualité et du checksum
					...
					if(angle>0 && angle <=90){
						if(distance>200 && distance <=2000)
						{
							...
							Driver_USART1.Send((const void*)tabDist,strlen(tabDist));										// envoi 
						...
						while(Driver_USART1.GetStatus().tx_busy == 1); 															// attente buffer TX vide
						Driver_USART1.Send((const void*)tabAngle,strlen(tabAngle));									// envoi
					}
					...
					if(angle>90 && angle <=135){
						...
						{
							sprintf(tabDist,"dist %5d mm ", distance);
							sprintf(tabQual,"qualite %0x  ", qualite);
							
							while(Driver_USART1.GetStatus().tx_busy == 1); 											// attente buffer TX vide
							Driver_USART1.Send((const void*)tabDist,strlen(tabDist));										// envoi
							while(Driver_USART1.GetStatus().tx_busy == 1); 											// attente buffer TX vide
							Driver_USART1.Send((const void*)tabQual,strlen(tabQual));										// envoi
						}
